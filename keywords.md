...
- **Codifica del programma**: scelta del linguaggio di programmazione.
...
- **Server-Side Languages (come Node.js, Python, PHP, Ruby)**: La capacità di sviluppare applicazioni server-side è cruciale per la gestione dei dati e la logica di business.
...
- **Database Management System**: Sistema software per la creazione, la manipolazione e l'interrogazione di una o più basi di dati in modo corretto ed efficiente.
...
- **Cherry-pick**: selezionare specifici commit e applicarli ad un altro branch
...
- **Static Analysis**: L'analisi statica del codice per identificare potenziali errori o problemi senza eseguire effettivamente il programma.
...
- **KISS**: (Keep it simple, stupid) Preferire soluzioni semplici e chiare rispetto a soluzioni complesse.
...
- **General data protection regulation**: (GDPR) Legge europea sulla protezione dei dati personali e della privacy.
...
- **Profiling**: Analisi delle prestazioni del codice per identificare le aree critiche in termini di tempo di esecuzione e utilizzo delle risorse.
...
- **Content Delivery Network**: (CDN) Una rete di server distribuiti globalmente per fornire contenuti web in modo rapido e affidabile.
...
- **Guest OS**: (Guest Operating System): Sistema operativo in esecuzione all'interno di una macchina virtuale.